...
func (s *Server) Run() {
	...
	router.HandleFunc("/api/v1/ingredients/{id}/stock", makeHTTPHandleFunc(apiIngredients.handleStockPUT)).Methods("PUT")
	...
	router.HandleFunc("/api/v1/diets/{id}/export", makeHTTPHandleFunc(apiDiets.handleExportByID)).Methods("GET")
	...
	router.HandleFunc("/api/v1/tools/ingredients-summary", makeHTTPHandleFunc(apiTools.handleIngredientSummaryPOST)).Methods("POST")
	...
	apiShoppingList := NewHandlerShoppingList()
