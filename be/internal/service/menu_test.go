...
func Test_MenuService_IsInTwoWeeksWindow(t *testing.T) {
	s := NewServiceMenu()

	date := time.Now().Truncate(24*time.Hour).AddDate(0, 0, -10)
	inWindow := s.IsInTwoWeeksWindow(date)
	t.Log("inWidnow: ", inWindow)
}
